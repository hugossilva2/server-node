...
> **Note:** in this repository the skill is already vendored at
> `.claude/skills/threads-carousel/`, so Claude Code picks it up automatically for
> anyone who clones the repo — only step 2 below is needed.

1. (Standalone install) Clone the repo into your Claude skills directory:
   ```bash
...
2. Install template dependencies (one-time):
   ```bash
   cd .claude/skills/threads-carousel/template   # or ~/.claude/skills/threads-carousel/template

...
This skill is installed inside the repo at `.claude/skills/threads-carousel/`, so resolve
`SKILL_DIR` from the repo root (fall back to the user-level install if it isn't there):

```bash
SKILL_DIR="$(git rev-parse --show-toplevel)/.claude/skills/threads-carousel"
[ -d "$SKILL_DIR" ] || SKILL_DIR="$HOME/.claude/skills/threads-carousel"

# One-time: install template deps (skip if node_modules already exists)
[ -d "$SKILL_DIR/template/node_modules" ] || (cd "$SKILL_DIR/template" && bun install)
